...
| 6 | **Estrutura** | Python 3.13. **Copiar** `token.py`/`lexer.py` da Parte 1 para dentro do projeto (zip autocontido). Sem executável empacotado; roda com `python3 src/main.py arquivo.arj` (ou `python3 run.py arquivo.arj`). Código em `src/`, entradas em `exemplos/`, documentos em `docs/` (reorganização de diretórios feita após a implementação). |
...
Estrutura de diretórios (reorganizada após a implementação):

```
analisador-sintatico/
├── README.md               # como executar + nota sobre a GLC
├── run.py                  # atalho: python3 run.py exemplos/ExemploArjanov.arj
├── src/                    # código-fonte
│   ├── token.py  lexer.py  grammar.py  slr.py  parser.py  main.py
├── exemplos/               # entradas .arj
│   ├── ExemploArjanov.arj  erros.arj
├── docs/                   # documentos
│   ├── DECISOES.md  GLC.md  trabalho_parte2.pdf  [GLC_Arjanov_SLR.pdf]
└── venv/
```

| Arquivo | Papel |
|---------|-------|
| `src/token.py` | Definições de `TokenType`, `Token`, `KEYWORDS` (copiado da Parte 1). |
| `src/lexer.py` | Analisador léxico (copiado; com o ajuste #9). |
| `src/grammar.py` | GLC simplificada como dado: produções numeradas, terminais, não-terminais, símbolo inicial, NTs de recuperação, mapeamento `Token`→terminal. |
| `src/slr.py` | Construção SLR(1): itens LR(0), `closure`/`goto`, coleção canônica, FIRST/FOLLOW, tabelas ACTION/GOTO, detecção de conflitos. |
| `src/parser.py` | Autômato de pilha shift/reduce, recuperação em modo pânico, construção da árvore e do traço. |
| `src/main.py` | Pipeline CLI: arquivo → léxico → tokens → parser → relatório. |
| `run.py` | Atalho na raiz (ajusta `sys.path` p/ `src/token.py` sombrear a stdlib). |
| `docs/GLC.md` | Gramática final para exportar em PDF. |
| `exemplos/ExemploArjanov.arj` | Entrada completa (todas as primitivas). |
| `exemplos/erros.arj` | Entrada com erros sintáticos (demonstra modo pânico). |
